...
export function openCryptoStudio() {
  ...
  el.cryptoStudioContent.innerHTML = `
    <div class="p-5 sm:p-6 space-y-5 font-mono text-xs overflow-y-auto max-h-[85vh]">
      ...
      <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
        <div class="flex items-center gap-2">
          ...
          <h3 class="text-base font-bold text-slate-900 dark:text-white">Crypto Studio & DID Playground</h3>
        </div>
        <button id="studio-close-btn" class="text-slate-400 hover:text-white p-1">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
        </button>
      </div>

      <!-- Section 1: DID Key Decoder -->
      <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-3">
        <h4 class="font-bold text-sm text-[#00c2ff]">1. Multicodec DID Decoder</h4>
        <p class="text-slate-400 text-xs font-sans">Paste any Ed25519 multibase DID to extract its raw 32-byte public key.</p>
        
        <input
          type="text"
          id="studio-did-input"
          placeholder="did:key:z6Mkq..."
          class="w-full p-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-100 text-xs font-mono focus:outline-none focus:border-[#00c2ff]"
        />

        <div id="studio-did-output" class="p-2.5 rounded-xl bg-slate-200 dark:bg-slate-900/60 text-slate-300 text-xs break-all hidden">
          <!-- Injected via JS -->
        </div>
      </div>
      ...
      <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-3">
        <h4 class="font-bold text-sm text-emerald-400">2. Offline Signature Proof Tester</h4>
        <p class="text-slate-400 text-xs font-sans">Verify any payload string <code class="text-cyan-300">room|nonce|text</code> against an Ed25519 signature directly in memory.</p>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label class="text-[10px] text-slate-400 uppercase">Room Name</label>
            <input type="text" id="studio-test-room" value="${escapeHtml(state.currentRoom)}" class="w-full mt-1 p-2 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-100 text-xs" />
          </div>
          <div>
            <label class="text-[10px] text-slate-400 uppercase">Nonce</label>
            <input type="text" id="studio-test-nonce" placeholder="e.g. 1718000000" class="w-full mt-1 p-2 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-100 text-xs" />
          </div>
        </div>

        <div>
          <label class="text-[10px] text-slate-400 uppercase">Message Text</label>
          <textarea id="studio-test-text" rows="2" placeholder="Message content..." class="w-full mt-1 p-2 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-100 text-xs font-mono"></textarea>
        </div>

        <div>
          <label class="text-[10px] text-slate-400 uppercase">Signer DID</label>
          <input type="text" id="studio-test-did" placeholder="did:key:z6Mk..." class="w-full mt-1 p-2 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-100 text-xs" />
        </div>

        <div>
          <label class="text-[10px] text-slate-400 uppercase">Signature (Hex or Base64url)</label>
          <input type="text" id="studio-test-sig" placeholder="64-byte Ed25519 signature" class="w-full mt-1 p-2 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-100 text-xs" />
        </div>

        <button id="studio-run-verify-btn" class="w-full py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-xl text-xs font-mono transition">
          Run Verification Engine
        </button>

        <div id="studio-verify-result" class="p-3 rounded-xl hidden"></div>
      </div>
