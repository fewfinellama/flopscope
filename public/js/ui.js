...
export async function openAgentDrawer(did) {
      ...
      <div class="flex items-start gap-4">
        ...
        <div class="min-w-0 flex-1 space-y-1">
          ...
          <div class="flex flex-wrap items-center gap-2 mt-2">
            <button id="drawer-copy-did-btn" class="btn-interactive flex-shrink-0 whitespace-nowrap px-2.5 py-1.5 bg-cyan-50 dark:bg-cyan-950/60 text-cyan-700 dark:text-[#00c2ff] border border-cyan-200 dark:border-cyan-800/80 rounded-xl text-xs font-mono font-bold flex items-center gap-1.5 transition-colors">
            ...
            <button id="drawer-watch-did-btn" class="btn-interactive flex-shrink-0 whitespace-nowrap px-2.5 py-1.5 ${state.watchedDids.has(did) ? 'bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-400 border-indigo-200 dark:border-indigo-800/80' : 'bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-800'} border rounded-xl text-xs font-mono font-bold flex items-center gap-1.5 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 transition-colors">
  ...
  const watchBtn = document.getElementById('drawer-watch-did-btn');
  watchBtn.onclick = () => {
    if (state.watchedDids.has(did)) {
      state.watchedDids.delete(did);
      showToast('Stopped watching DID');
      watchBtn.className = "btn-interactive flex-shrink-0 whitespace-nowrap px-2.5 py-1.5 bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-800 border rounded-xl text-xs font-mono font-bold flex items-center gap-1.5 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors";
      watchBtn.innerHTML = `<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg><span>Watch</span>`;
    } else {
      state.watchedDids.add(did);
      showToast('Watching DID');
      watchBtn.className = "btn-interactive flex-shrink-0 whitespace-nowrap px-2.5 py-1.5 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-400 border-indigo-200 dark:border-indigo-800/80 border rounded-xl text-xs font-mono font-bold flex items-center gap-1.5 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 transition-colors";
      watchBtn.innerHTML = `<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg><span>Watching</span>`;
  ...
  el.modalContainer.innerHTML = `
    <div class="p-5 sm:p-6 space-y-4 font-mono text-sm">
      ...
      <div class="flex items-center justify-between">
        ...
        <button id="raw-json-copy-btn" class="btn-interactive px-2.5 py-1.5 bg-cyan-50 dark:bg-cyan-950/60 text-cyan-700 dark:text-[#00c2ff] border border-cyan-200 dark:border-cyan-800 rounded-xl font-bold flex items-center gap-1.5">
